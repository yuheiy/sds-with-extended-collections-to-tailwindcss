...
const tailwindTokensMap = {
  ...Object.fromEntries(themeTokensEntries),
  ...Object.fromEntries([['./packages/ui/tokens/components.tokens.json', componentsTokens]]),
};
...
function rewriteObject<T extends Record<string, unknown>>(object: T): T {
  return mapObject(
    object,
    (key, value) => {
...
function rewriteToken(token: ExtractToken<typeof figmaTokens>) {
  if (typeof token.$value === 'object') {
    token.$value = Array.isArray(token.$value)
      ? token.$value.map((element) => (isObject(element) ? rewriteObject(element) : element))
      : rewriteObject(token.$value);
  } else if (typeof token.$value === 'string' && token.$value.includes('{')) {
    token.$value = rewriteReferences(token.$value);
  }

  if ('$extensions' in token && 'mode' in token.$extensions) {
    token.$extensions.mode = rewriteObject(token.$extensions.mode);
  }
}
...
for (const [file, tokens] of Object.entries(tailwindTokensMap)) {
